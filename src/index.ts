...
async function parse(pathToFolder) {
    ...
    await glob(
        PATTERNS,
        { cwd: pathToFolder, ignore: IGNORE },
        async (err, files) => {
            if (err) {
                console.log(err);
                return;
            ...
            const pathToFileList = files.map(pathToFile =>
                pathToFile.split('/'),
            );
            const promises = pathToFileList.map(pathToFile => async () => {
                return await Promise.all(
                    pathToFile.flatMap(async (folderOrFile, index) => {
            ...
            const nodes = new Set(
                [...map.keys()].map(value => normalize(value, pathToFolder)),
            );
            nodes.forEach(value =>
                addNode([
                    {
